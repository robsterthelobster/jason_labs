...
class VectorN(object):
	...
	def dot(self,vector):
		"""Calculates the dot product of these two vectors.
		   Reminder:
		      self.dot(vector) == self.magnitude * vector.magnitude * cos(theta)"""
		if isinstance(vector, VectorN) and self.mDim == vector.mDim:
			tmp = 0
			for i in range(self.mDim):
				tmp += self.mData[i] * vector.mData[i]
			return tmp
		else:
			raise TypeError("Must have a " + str(self.mDim) + "VectorN to get the dot product." )